import { d as defineEventHandler, a as getQuery, b as useRuntimeConfig, c as createError } from '../../../../nitro/nitro.mjs';
...
const _____ = defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.log("\u{1F504} Fetching classname from:", url);
    const response = await $fetch(url, {
      headers: {
        "X-API-Key": config.dzsvApiKey
      }
    });
    console.log("\u{1F4E6} Raw API Response:", JSON.stringify(response).substring(0, 200) + "...");
    if (path === "search") {
      let classnames = [];
      ...
      return {
        success: true,
        results: classnames
        // Return ONLY array of classname strings
      };
    }
    if (path === "categories") {
      return {
    ...
    if (path === "search") {
      ...
      const filtered = mockClassnames.filter(
        (cn) => cn.toLowerCase().includes(searchQuery)
      );
      console.log("\u{1F50D} Mock search for:", searchQuery, "- Found:", filtered.length, "items");
      return {
        success: true,
